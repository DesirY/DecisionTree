...
# replace unknow with..
def get_unknown_map(examples):
    unknown_map = {
        'job': '', 'education': '', 'contact': '', 'month': '', 'poutcome': '', 'marital': ''
    }
    for key in unknown_map.keys():
        values = []
        for example in examples:
            value = example[key]
            if value != "unknown":
                values.append(value)
        max_value = max(values, key=values.count)
        unknown_map[key] = max_value
    return unknown_map


# map unknown attribute to the most common one
def init_unknown_attr(unknown_attr_map, examples):
    for example in examples:
        for key in unknown_attr_map.keys():
            if example[key] == 'unknown':
                example[key] = unknown_attr_map[key]
...
if __name__ == '__main__':
    ...
    unknown_map = get_unknown_map(train_examples)
    train_examples = init_unknown_attr(unknown_map, train_examples)
    
    test_examples = process_data('./data/bank/test.csv')
    test_examples = num_to_binary(numerical_media_map, test_examples)
    test_examples = init_unknown_attr(unknown_map, test_examples)
